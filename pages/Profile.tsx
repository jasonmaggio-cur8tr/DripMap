...
const Profile: React.FC = () => {
  ...
  const { user: currentUser, logout, shops, updateUserProfile, getProfileById, toggleFollow } = useApp();
  ...
  useEffect(() => {
    ...
    fetchProfile();
  }, [id, currentUser, isOwnProfile, navigate, getProfileById]);
  ...
  const handleToggleFollow = () => {
      ...
      toggleFollow(viewedUser.id);
      // Force re-render logic handled by context update usually, but for mock we rely on optimistic UI in a real app
      toast.success(currentUser.following.includes(viewedUser.id) ? "Unfollowed" : "Following!");
  };
  ...
  return (
    <div className="min-h-screen bg-coffee-50 pt-20 px-4 pb-20">
      <div className="container mx-auto max-w-4xl">
         ...
         {/* Header Card */}
         <div className="bg-white rounded-3xl shadow-lg p-8 border border-coffee-100 flex flex-col md:flex-row items-start gap-8 relative mb-8">
            
            {/* Avatar Section */}
            <div className="flex-shrink-0 mx-auto md:mx-0 relative group">
                <div className="w-28 h-28 rounded-full overflow-hidden border-4 border-coffee-100 shadow-sm">
                    <img 
                ...
                {isEditing ? (
                    ...
                    <>
                        <div className="flex flex-col md:flex-row md:items-center gap-3 mb-2 justify-center md:justify-start">
                            <h1 className="text-3xl font-bold text-coffee-900">@{viewedUser.username}</h1>
                            
                            {/* Action Buttons */}
                            <div className="flex gap-2">
                                <button 
                                    onClick={handleShareProfile}
                                    className="w-8 h-8 rounded-full bg-coffee-50 text-coffee-600 hover:bg-coffee-100 flex items-center justify-center transition-colors"
                                    title="Share Profile"
                                >
                        ...
                        <div className="mb-4 flex justify-center md:justify-start">
                            <div className="flex items-center gap-1 bg-coffee-900 text-volt-400 px-3 py-1 rounded-full text-xs font-bold shadow-lg">
                        ...
                        {viewedUser.bio ? (
                            <p className="text-coffee-700 italic mb-4 max-w-lg mx-auto md:mx-0">"{viewedUser.bio}"</p>
                        ) : (
                            <p className="text-coffee-400 text-sm mb-4">No bio yet.</p>
                        )}
                        ...
                        <div className="flex justify-center md:justify-start gap-6 mb-6 pt-4 border-t border-coffee-50 md:border-none">
                            <div className="text-center md:text-left">
                                <span className="block font-bold text-xl text-coffee-900">{viewedUser.followers.length}</span>
                                <span className="text-xs text-coffee-500 uppercase tracking-wide">Followers</span>
                            </div>
                            <div className="text-center md:text-left">
                                <span className="block font-bold text-xl text-coffee-900">{viewedUser.following.length}</span>
                                <span className="text-xs text-coffee-500 uppercase tracking-wide">Following</span>
                            </div>
                             <div className="text-center md:text-left pl-6 border-l border-coffee-100">
                                <span className="block font-bold text-xl text-coffee-900">{savedSpots.length}</span>
                                <span className="text-xs text-coffee-500 uppercase tracking-wide">Saved</span>
                            </div>
                            <div className="text-center md:text-left">
                                <span className="block font-bold text-xl text-coffee-900">{visitedSpots.length}</span>
                                <span className="text-xs text-coffee-500 uppercase tracking-wide">Visited</span>
                            </div>
                        </div>
                        ...
                        {isOwnProfile && (
                            <div className="flex gap-3 justify-center md:justify-start">
                                ...
                                <Button variant="ghost" size="sm" onClick={() => { logout(); navigate('/'); }} className="text-red-500 hover:bg-red-50">
                                    Log Out
                                </Button>
                            </div>
                        )}
         ...
         <div className="mb-10">
             <div className="flex items-center gap-3 mb-4">
                <h2 className="text-xl font-serif font-bold text-coffee-900">Achievements</h2>
             </div>
         ...
         <div className="mb-10">
            <h2 className="text-xl font-serif font-bold text-coffee-900 mb-4 flex items-center gap-2">
                <i className="fas fa-passport text-coffee-400"></i> Passport Book
            </h2>
            
            {Object.keys(passportBook).length > 0 ? (
                <div className="bg-[#FDFBF7] border-2 border-coffee-900 rounded-3xl p-6 md:p-10 shadow-2xl relative overflow-hidden">
                    ...
                    {Object.entries(passportBook).map(([location, shops]) => (
                ...
                <div className="bg-coffee-50 border-2 border-dashed border-coffee-200 rounded-3xl p-12 text-center">
                     ...
                     <h3 className="text-lg font-bold text-coffee-900 mb-2">Passport is empty</h3>
                </div>
            )}
         </div>
         ...
         {claimedSpots.length > 0 && (
             <div className="mt-10 pb-10">
                <h2 className="text-xl font-serif font-bold text-coffee-900 mb-4 flex items-center gap-2">
                    <i className="fas fa-briefcase text-coffee-400"></i> Managed Shops
                </h2>
         ...
         <div className="mt-10 pb-20">
            <h2 className="text-xl font-serif font-bold text-coffee-900 mb-4 flex items-center gap-2">
                <i className="fas fa-heart text-coffee-400"></i> Saved Spots
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
